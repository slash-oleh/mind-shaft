...
export const PKG_ROOT = 'packages/rules';
export const SRC_DIR = 'src';
export const DIST_DIR = 'dist';
export const AI_RULES_DIR = path.join(DIST_DIR, 'ai');
...
export const stripFrontMatter = (content) =>
  content.replace(FRONT_MATTER_RE, '');
...
export const extractSolutions = (content) => {
  ...
  for (const sectionName of ['Good Solution', 'Bad Solution']) {
    ...
    if (sectionNodes && sectionNodes.length > 0) {
      ...
      result[key] = stripped.slice(start, end).trim();
    }
  }
