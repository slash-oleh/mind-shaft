...
import { extractFirstParagraph, walkArticles, formatName, DIST_DIR, SRC_DIR } from './common.js';
...
const generateToc = async () => {
  ...
  await walkArticles(
    async ({ cat, catLabel, subDir, subDirLabel, article, content }) => {
      ...
      if (paragraph) {
        const relPath = path.posix.join('..', SRC_DIR, cat.name, subDir.name, article.name);
