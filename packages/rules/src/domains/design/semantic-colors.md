...
- **Maintainability**: Changing a brand color requires updating a single mapping in the theme configuration, rather than renaming classes across hundreds of components.
- **Robustness**: A semantic naming architecture is the foundational requirement for supporting multiple themes (like Light/Dark modes) efficiently without duplication.
- **Explicitness**: A token name like `danger` communicates _why_ the color is used, whereas `red` only communicates _what_ it currently looks like.
