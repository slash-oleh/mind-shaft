...
- **Consistency**: A mathematical multiplier system establishes a strong, uniform visual rhythm across the entire application interface.
- **Maintainability**: If the global spacing tightness needs to change, you update the base factor behind the token, rather than changing 5,000 individual `px` declarations.
- **KISS**: Developers don't have to guess or argue about whether a margin should be `15px` or `16px`; they simply choose the next step on the `4px` token scale.
