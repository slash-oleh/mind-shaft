...
- **Maintainability**: Adding a new required property from the same object doesn't break the function signature.
- **Reusability**: Functions accepting domain types are easier to compose and share across modules.
- **Robustness**: Reduces the risk of argument-order errors, especially when multiple parameters share the same type.
- **Single Source of Truth**: Logic remains coupled to the data structure it operates on.
