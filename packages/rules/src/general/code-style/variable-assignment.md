...
- **Declarative**: Explicitly defines _what_ the value is based on a condition, rather than _how_ to set it.
- **Readability**: Reduces the amount of code and keeps the initialization in one place.
- **Stateless**: Encourages immutability by allowing the use of `const`.
- **Size/Code Amount**: significantly shorter than the equivalent `if/else` structure.
