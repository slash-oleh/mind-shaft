...
- **Maintainability**: Additive CSS is much easier to maintain than destructive CSS. You don't have to trace a long chain of overrides to figure out where a color is coming from.
- **Performance**: The browser spends less time computing styles when it doesn't have to evaluate and discard overridden properties.
- **KISS**: Reduces cognitive load linearly; a class simply applies what it says, rather than fighting against base styles.
