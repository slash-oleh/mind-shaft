...
- **Maintainability**: Changing a hardcoded value in the application code is significantly easier and safer than performing a database schema migration.
- **Explicitness**: Developers can see the default values immediately when inspecting the domain models in the codebase.
- **Cohesion**: Business rules regarding default states are kept closely aligned within the domain layer, rather than leaking into the database infrastructure layer.
