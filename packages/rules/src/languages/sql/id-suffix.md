...
- **Explicitness**: The `_id` suffix immediately tells the developer that the field holds a scalar identifier, not a serialized object or populated relationship.
- **Maintainability**: Aligns with universal relational database conventions and prevents conflicts with ORMs that need to expose both the ID and the fetched relationship object on the same model.
