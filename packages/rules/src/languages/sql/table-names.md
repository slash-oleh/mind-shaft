...
- **Consistency**: Singular nouns avoid the linguistic gymnastics of irregular plurals, ensuring every table follows the exact same naming pattern.
- **KISS**: Easier to map table names directly to application models classes (e.g., table `user` -> class `User`).
- **Readability**: Queries often feel more natural when referring to an alias of a single row (`SELECT user.id FROM user`).
