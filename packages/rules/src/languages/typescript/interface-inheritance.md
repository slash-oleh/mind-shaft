...
- **Maintainability**: Common fields are defined in a single source of truth.
- **Scalability**: New subtypes can be added without modifying existing functions that operate on the base interface (Open/Closed Principle).
- **Consistency**: Ensures that all related entities strictly follow the same base structure.
- **Reliability**: Reduces the risk of "missing" a field update in one of the union branches.
