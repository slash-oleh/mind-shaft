...
- **Performance**: Vital for making `React.memo` effective by ensuring props remain referentially equal between renders.
- **Robustness**: Prevents `useEffect` from re-running unnecessarily when the callback logic itself hasn't changed.
