...
- **Performance**: Stable keys allow React to reuse existing DOM nodes instead of destroying and recreating them.
- **Robustness**: Ensures that local component state (like text in an input) remains attached to the correct data item even as the list moves.
- **Consistency**: Prevents visual glitches and ensures transitions work as expected during list modifications.
