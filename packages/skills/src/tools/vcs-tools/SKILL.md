...
Prints, in order: title/body, URL, merge state, failed CI checks/jobs (each with a file path to its filtered log lines), reviews/approvals, open (unresolved) review threads with `thread_id`, `location`, `author`, and `comments`, and source/target branch names.
...
#### `post-reply <PR_NUMBER> <THREAD_ID> <BODY_FILE>`

`THREAD_ID` comes from `get-pr-info`'s Open Review Threads output.
