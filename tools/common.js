  ...
  for (const category of categories) {
    ...
    for (const subDir of subDirs) {
      ...
      const articles = (await readdir(subDirPath, { withFileTypes: true }))
        .filter(f => f.isFile() && f.name.endsWith('.md') && !IGNORE_FILES.includes(f.name))
        .sort((a, b) => a.name.localeCompare(b.name));
...
export const extractSolutions = (content) => {
  ...
  for (const sectionName of sections) {
    const heading = children.find(n => n.type === 'heading' && n.depth === 2 && toString(n).toLowerCase() === sectionName.toLowerCase());
    if (heading) {
