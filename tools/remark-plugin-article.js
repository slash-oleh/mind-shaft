...
const validateTLDR = (nodes, file, headingNode, settings) => {
  ...
  if (paragraphs.length !== 1 || nodes.length !== 1) {
    warn(
      file,
      'Section "## TLDR" must contain exactly one paragraph and no other content',
      headingNode.position,
      'checkSingleParagraph',
      settings,
      'tldr',
    );
  }
...
const validateProblem = (nodes, file, headingNode, settings) => {
  if (!nodes.some((n) => n.type === 'paragraph')) {
    warn(
      file,
      'Section "## Problem" must contain at least one paragraph',
      headingNode.position,
      'checkParagraph',
      settings,
      'problem',
...
const validateSolution = (
  ...
  if (!hasParagraph) {
    warn(
      file,
      `Section "## ${title}" must contain a description paragraph`,
      headingNode.position,
      'checkParagraph',
      settings,
      configKey,
  ...
  if (codeBlocks.length) {
    warn(
      file,
      `Section "## ${title}" must contain at least one code block`,
      headingNode.position,
      'checkCode',
      settings,
      configKey,
...
const validateImpact = (nodes, file, headingNode, settings) => {
  ...
  if (nonListNodes.length > 0) {
    warn(
      file,
      'Section "## Impact" must contain only a list',
      headingNode.position,
      'checkOnlyList',
      settings,
      'impact',
  ...
  if (!list) {
    warn(
      file,
      'Section "## Impact" must contain a list',
      headingNode.position,
      'checkList',
      settings,
      'impact',
    ...
    list.children.forEach((item) => {
      ...
      if (!paragraph) {
        warn(
          file,
          '"Impact" list items must contain text',
          item.position,
          'checkListItemText',
          settings,
          'impact',
...
const validateListOnly = (
  ...
  if (nodes.length !== 1 || nodes[0].type !== 'list') {
    warn(
      file,
      `Section "## ${title}" must contain only a list`,
      headingNode.position,
      'checkOnlyList',
      settings,
      configKey,
...
const validateReferences = (nodes, file, headingNode, settings) => {
  if (nodes.length !== 1 || nodes[0].type !== 'list') {
    warn(
      file,
      'Section "## References" must contain only a list',
      headingNode.position,
      'checkOnlyList',
      settings,
      'references',
    ...
    list.children.forEach((item) => {
      ...
      if (!paragraph) {
        warn(
          file,
          '"References" list items must contain a link',
          item.position,
          'checkLink',
          settings,
          'references',
      ...
      const hasOtherContent = paragraph.children.some(
        (c) =>
      ...
      if (!link) {
        ...
        warn(
          file,
          '"References" list items must contain only a link',
          item.position,
          'checkOnlyLink',
          settings,
          'references',
        ...
        if (!linkText.includes(': ')) {
          warn(
            file,
            `"References" link text "${linkText}" must follow the format: "[Source: Title by Author]"`,
            item.position,
            'checkFormat',
            settings,
            'references',
        ...
        if (!isExternal && isMarkdown) {
          warn(
            file,
            'Do not reference other internal articles in the "References" section',
            item.position,
            'checkInternal',
            settings,
            'references',
...
const checkH1 = (children, filePath, file, settings) => {
  ...
  if (actualFileName !== expectedFileName) {
    warn(
      file,
      `Filename "${actualFileName}" should match hyphenated title "${expectedFileName}"`,
      h1.position,
      'checkFileNameMatch',
      settings,
    );
  ...
  if (
    ...
    nextNode.depth !== 2 ||
    toString(nextNode).toLowerCase() !== 'tldr'
  ) {
    warn(
      file,
      'H1 title must be followed by a "## TLDR" section',
      h1.position,
      'checkH1Description',
      settings,
    );
...
const checkH2Sections = (children, file, tree, settings) => {
  ...
  const titleToKey = Object.fromEntries(
    sectionEntries.map(([key, { title }]) => [title, key]),
  );
  ...
  h2s.forEach((node) => {
    ...
    if (allowedIndex < lastAllowedIndex) {
      warn(
        file,
        `Section "## ${title}" is in the wrong position`,
        node.position,
        'checkSectionOrder',
        settings,
      );
...
const remarkArticleStructure =
  (options = {}) =>
  (tree, file) => {
    const sections = Object.fromEntries(
      Object.entries(DEFAULT_SETTINGS.sections).map(
    ...
    const children = tree.children.filter(
      (n) => n.type !== 'html' || !n.value.startsWith('<!--'),
    );
